...
| Server "restarted" but still serves OLD code / new feature missing | Windows double-bind: pre-2026-07-10 builds let a second `import_server.py`/`rag.py serve` silently bind the SAME port (SO_REUSEADDR) — the stale process keeps answering. Diagnose: `Get-CimInstance Win32_Process -Filter "Name='python.exe'"` and look for duplicates; kill ALL of them, start ONE. Current builds bind exclusively and the second start exits loudly — but long-running servers started before an upgrade still need a manual restart to pick it up |
